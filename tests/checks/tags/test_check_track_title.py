...
class TestCheckTrackTitle:
    def test_check_track_title_ok(self):
        album = Album(
            "Foobar" + os.sep,
            [
                Track("1 foo.mp3", {BasicTag.TITLE: ["foo"]}),
                Track("2 bar.mp3", {BasicTag.TITLE: ["bar"]}),
                Track("3 baz.mp3", {BasicTag.TITLE: ["baz"]}),
            ],
        )
    ...
    def test_check_track_title_guess_some(self, mocker):
        album = Album(
            "Foobar" + os.sep,
            [
                Track("1 foo.flac", {BasicTag.TITLE: ["foo"]}),
                Track("2 bar.flac"),
                Track("3.flac"),
            ],
        )
    ...
    def test_check_track_title_with_disc_number(self, mocker):
        album = Album(
            "Foobar" + os.sep, [Track("1 foo.flac", {BasicTag.TITLE: ["foo"]}), Track("2 bar.flac"), Track("3 baz.flac", {BasicTag.TITLE: ["baz"]})]
        )
        result = CheckTrackTitle(Context()).check(album)
        assert result is not None
