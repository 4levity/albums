...
from albums.checks.picture.check_cover_selection import CheckCoverSelection
from albums.types import Album, Picture, PictureType, Stream, Track

from ...fixtures.create_library import create_library, make_image_data


class TestCheckCoverSelection:
    def test_cover_ok(self):
        ...
        assert not CheckCoverSelection(Context()).check(album)
    ...
    def test_cover_missing_required(self):
        album = Album("", [Track("1.flac", {}, 0, 0, Stream(1.5, 0, 0, "FLAC")), Track("2.flac", {}, 0, 0, Stream(1.5, 0, 0, "FLAC"))])
        ctx = Context()
        ctx.config.checks = {CheckCoverSelection.name: {"cover_required": True}}
        result = CheckCoverSelection(ctx).check(album)
    ...
    def test_cover_multiple_unique(self):
        ...
        result = CheckCoverSelection(Context()).check(album)
        assert result is not None
        assert result.message == "COVER_FRONT pictures are not all the same"

    def test_cover_multiple_unique_allowed(self):
        album = Album(
            "",
            [
                Track("1.flac", {}, 0, 0, Stream(1.5, 0, 0, "FLAC"), [Picture(PictureType.COVER_FRONT, "image/png", 400, 400, 0, b"")]),
        ...
        ctx = Context()
        ctx.config.checks = {CheckCoverSelection.name: {"unique": False}}
        result = CheckCoverSelection(ctx).check(album)
    ...
    def test_album_pictures_but_no_front_cover(self, mocker):
        ...
        mock_get_embedded_image_data = mocker.patch("albums.checks.picture.check_cover_selection.get_embedded_image_data", return_value=[image_data])
    ...
    def test_album_picture_files_no_front_cover(self, mocker):
        ...
        mock_rename = mocker.patch("albums.checks.picture.check_cover_selection.rename")
        result.fixer.fix(result.fixer.options[result.fixer.option_automatic_index])
    ...
    def test_has_unmarked_cover_source_file(self, mocker):
        ...
        update_picture_files_mock = mocker.patch("albums.checks.picture.check_cover_selection.operations.update_picture_files")
