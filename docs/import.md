...
specified, it applies automatic fixes, then stops at every failed check.
...
importing an album, those attributes are not imported. So those checks may be
flagged again, and files you marked as cover source may trigger the checks
again, the next time the library is checked.
