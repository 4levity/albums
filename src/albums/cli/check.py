...
@click.option("--timing", is_flag=True, help="report per-check wall-clock timing (init, pass and fail) after the run")  # pyright: ignore[reportUnknownMemberType]
...
@pass_context
def check(ctx: Context, default: bool, automatic: bool, fix: bool, interactive: bool, timing: bool, checks: list[str]):
    ...
    checker = Checker(ctx, automatic, fix, interactive, show_ignore_option=ctx.is_persistent, timing=timing)
    if len(checks) > 0:
